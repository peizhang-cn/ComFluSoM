...
namespace MetaballFunctions
{
    ...
    void FindClosestPointFromMetaballToMetaball(vector<Vector3d>& metaPi, vector<Vector3d>& metaPj, VectorXd& metaKi, VectorXd& metaKj, double step, Vector3d& initP, Vector3d& cp, bool& convFlag)
    {
        ...
        for (size_t k=0; k<nk; ++k)
        {
            ...
            if (fjt.norm()<1e-7 && abs(ci-1.)<1.e-6)
            {
                // cout << "k: " << k << endl;
                convFlag = true;
                break;
        ...
        for (size_t i=0; i<p0.size(); ++i)
        {
            ...
            if (sideCheck<0.)
            {
                ...
                // if (cm>cb && cm>ci)
                {
                    ...
                    // cout << "isline" << endl;
                    Vector3d point_s = p0[i];
                    Vector3d point_e = p0[id];

                    double c_s; Vector3d f_s;
                    CalMetaCF(metaP, metaK, point_s, c_s, f_s);
                    double c_e; Vector3d f_e;
                    CalMetaCF(metaP, metaK, point_e, c_e, f_e);

                    f_s.normalize();
                    f_e.normalize();

                    double res_s = f_s.dot(vecn);
                    double res_e = f_e.dot(vecn);

                    int sign_s = (res_s>0.) ? 1 : -1;
                    int sign_e = (res_e>0.) ? 1 : -1;
                    ...
                    {
                        Vector3d point_m = 0.5*(point_s+point_e);
                        double cm; Vector3d fm;
                        CalMetaCF(metaP, metaK, point_m, cm, fm);
                        fm.normalize();
                        double res_m = fm.dot(vecn);
                        int sign_m = (res_m>0.) ? 1 : -1;
                        if (sign_m==sign_s)
                        {
                            point_s = point_m;
                            sign_s = sign_m;
                        }
                        else if (sign_m==sign_e)
                        {
                            point_e = point_m;
                            sign_e = sign_m;
                        }
