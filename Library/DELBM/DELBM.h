...
public:
	...
	void SloveOneStepIBB(int method, int demNt);
    void SloveOneStepNEBB(int demNt);
    void SloveOneStepVAM(int demNt, bool save, int ct);
    void SloveOneStepPSM(int demNt);
    void SloveVAM(int tt, int savet, int demNt);
    void CalRhoVDELBM();
...
inline void DELBM::MoveVAM(bool updatebox)
{
    #pragma omp parallel for schedule(static) num_threads(Nproc)
...
inline void DELBM::SloveOneStepVAM(int demNt, bool save, int ct)
{
    ...
    if (save)
    {
        // DomLBM->WriteFileH5(ct);
        DomLBM->WriteFileH5(ct, 2);
...
inline void DELBM::SloveVAM(int tt, int savet, int demNt)
{
    for (int t=0; t<tt; ++t)
    {
        bool save = false;
        if (t%savet==0)    save = true;
        SloveOneStepVAM(demNt, save, t);
    }
}

// inline void DELBM::SloveOneStepPSM(int demNt)
// {
//     DomLBM->CollideSRT();
//     ApplyPSM();
//     DomLBM->Stream();
...
inline void DELBM::SloveOneStepIBB(int method, int demNt)
{
...
inline void DELBM::WriteFileH5(int n, int scale)
{
    ...
    hsize_t dims_scalar_p[1] = {DomDEM->Lp.size()};           //create data space.
    hsize_t dims_vector_p[1] = {3*DomDEM->Lp.size()};         //create data space.
    ...
    double* r_p_h5    = new double[  DomDEM->Lp.size()];
    double* rho_p_h5  = new double[  DomDEM->Lp.size()];
    double* tag_p_h5  = new double[  DomDEM->Lp.size()];
    double* pos_p_h5  = new double[3*DomDEM->Lp.size()];
    double* vel_p_h5  = new double[3*DomDEM->Lp.size()];
    double* agv_p_h5  = new double[3*DomDEM->Lp.size()];
    double* fh_p_h5   = new double[3*DomDEM->Lp.size()];
    ...
    for (size_t i=0; i<DomDEM->Lp.size(); ++i)
    {
        Vector3d agv = DomDEM->Lp[i]->Q._transformVector(DomDEM->Lp[i]->W);
        r_p_h5  [  i  ]   = DomDEM->Lp[i]->R/(double) scale;
        rho_p_h5[  i  ]   = DomDEM->Lp[i]->Rho;
        tag_p_h5[  i  ]   = DomDEM->Lp[i]->Tag;
        pos_p_h5[3*i  ]   = DomDEM->Lp[i]->X(0)/(double) scale;
        pos_p_h5[3*i+1]   = DomDEM->Lp[i]->X(1)/(double) scale;
        pos_p_h5[3*i+2]   = DomDEM->Lp[i]->X(2)/(double) scale;
        vel_p_h5[3*i  ]   = DomDEM->Lp[i]->V(0);
        vel_p_h5[3*i+1]   = DomDEM->Lp[i]->V(1);
        vel_p_h5[3*i+2]   = DomDEM->Lp[i]->V(2);
        fh_p_h5[3*i  ]    = DomDEM->Lp[i]->Fh(0);
        fh_p_h5[3*i+1]    = DomDEM->Lp[i]->Fh(1);
        fh_p_h5[3*i+2]    = DomDEM->Lp[i]->Fh(2);
        agv_p_h5[3*i  ]   = agv(0);
        agv_p_h5[3*i+1]   = agv(1);
        agv_p_h5[3*i+2]   = agv(2);
    }
