...
#ifndef PARTICLE_PROPERTIES_CONVEX_H
#define PARTICLE_PROPERTIES_CONVEX_H
...
// Only works with convex faces
double CalSurfaceArea(vector<VectorXi>& F, vector<Vector3d>& P)
{
	double area = 0.;
	for (size_t i=0; i<F.size(); ++i)
	{
		Vector3d v0 = P[F[i](0)];					// the first vertice
		for (int j=0; j<F[i].size()-2; ++j)
		{
			Vector3d v1 = P[F[i](j+1)];				// the second vertice
			Vector3d v2 = P[F[i](j+2)];				// the third vertice
			double ai = ((v1-v0).cross(v2-v0)).norm()/2.;
			area += ai;
		}
	}
	return area;
